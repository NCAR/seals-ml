...
class Preprocessor(object):
    ...
    def load_data(self, files, remove_blind_samples=False, use_noise=False, noise_mean=1.5e-6, noise_std=1e-6):
        ...
        if use_noise:
            ch4_shape = encoder_data.sel(sensor=slice(1, 6), variable='q_CH4', mask=0).shape
            encoder_data.sel(sensor=slice(1, 6), variable='q_CH4', mask=0).values += np.random.normal(noise_mean,
                                                                                                      noise_std,
                                                                                                      size=ch4_shape)
