...
class LeakRateMetricsCallback(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        y_pred = self.model.predict(self.x_val, batch_size=self.batch_size)
        logs["val_rmse"] = np.sqrt(keras.metrics.mean_squared_error(self.y_val, y_pred[:, 0]).numpy())
        logs["val_mae"] = keras.metrics.mean_absolute_error(self.y_val, y_pred[:, 0]).numpy()
        logs["val_mean_error"] = mean_error(self.y_val, y_pred[:, 0]).numpy()
        logs["val_sharpness"] = sharpness(self.y_val, y_pred[:, 0]).numpy()
